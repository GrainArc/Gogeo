...
// ImportPostGISToGDBWithFilter 带过滤条件的PostGIS导入GDB
// whereClause: SQL WHERE条件，如 "status = 1" 或 "area > 1000"
func ImportPostGISToGDBWithFilter(postGISConfig *PostGISConfig, gdbPath string, gdbLayerName string, whereClause string, options *ImportToGDBOptions) (*ImportResult, error) {
	// 初始化GDAL
	InitializeGDAL()
	...
	// 1. 从PostGIS读取数据（带过滤条件）
	reader := NewPostGISReaderWithFilter(postGISConfig, whereClause)
	sourceLayer, err := reader.ReadGeometryTable()
	if err != nil {
		return nil, fmt.Errorf("读取PostGIS表失败: %v", err)
	}
	...
	for {
		...
		if sourceGeom != nil {
			...
			if clonedGeom != nil {
				// 坐标转换
				if needTransform && transform != nil {
					if C.OGR_G_Transform(clonedGeom, transform) != C.OGRERR_NONE {
						if options.SkipInvalidGeometry {
							C.OGR_G_DestroyGeometry(clonedGeom)
							C.OGR_F_Destroy(targetFeature)
							C.OGR_F_Destroy(sourceFeature)
...
// NewPostGISReaderWithFilter 创建带过滤条件的PostGIS读取器
func NewPostGISReaderWithFilter(config *PostGISConfig, whereClause string) *PostGISReaderFiltered {
	return &PostGISReaderFiltered{
		config:      config,
...
// BatchImportPostGISToGDB 批量导入多个PostGIS表到GDB
// tableMapping: PostGIS表名 -> GDB图层名的映射
func BatchImportPostGISToGDB(baseConfig *PostGISConfig, gdbPath string, tableMapping map[string]string, options *ImportToGDBOptions) (map[string]*ImportResult, error) {
	results := make(map[string]*ImportResult)

	for pgTable, gdbLayer := range tableMapping {
		fmt.Printf("\n开始导入: %s -> %s\n", pgTable, gdbLayer)

		// 创建当前表的配置
		tableConfig := &PostGISConfig{
		...
		// 执行导入
		result, err := ImportPostGISToGDB(tableConfig, gdbPath, gdbLayer, options)
		if err != nil {
			fmt.Printf("导入 %s 失败: %v\n", pgTable, err)
			results[pgTable] = &ImportResult{
				Errors: []string{err.Error()},
			}
			continue
		}

		results[pgTable] = result
		fmt.Printf("导入 %s 完成: 成功 %d 条\n", pgTable, result.InsertedCount)
	}
